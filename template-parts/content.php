...
$tipo_plantilla = function_exists('get_field') ? get_field('post_fact_checking_plantilla_fact-checking') : null;
?>

<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
   <div class="share-iconos"><?php echo do_shortcode('[Sassy_Social_Share type="floating"]') ?></div>
    <?php insertar_datos_estructurados(); ?>
        ...
        if (in_category('diver-check')) {
            ...
            if ($tipo_plantilla == 'new'):?>
                <div class="fact-checking-newbox p-4">
                    ...
                    <?php echo function_exists('get_field') ? get_field('post_fact_checking_lo_dicho') : ''; ?>
                </div>
                <?php
                if (in_category('diver-check')) {

                    $term = $_categories['children'][0];
                    if ($term instanceof WP_Term) { // Verificar si $term es un objeto WP_Term
                        $term_id = $term->term_id;  // Extraer el ID del término
                    } else {
                        $term_id = $term;  // Si ya es un ID, no hay problema
                    }

                    if ($term_id != null):  // CORREGIDO
                        $image_id = function_exists('get_field') ? get_field('category_logo', 'term_' . $term_id) : null; // Se usa 'term_ID' para taxonomía
                        $image_data = wp_get_attachment_image_src($image_id, 'full');
                        $image_src = is_array($image_data) ? $image_data[0] : ''; // URL de la imagen
                        $image_alt = $image_id ? get_post_meta($image_id, '_wp_attachment_image_alt', true) : ''; // Texto alternativo
                        $color = function_exists('get_field') ? get_field('category_color', 'term_' . $term_id) : '';
                        ?>
                        ...
                        <div class="fact-checking mt-2 text-center mx-auto mb-4"
                            ...
                            <img src="<?php echo $image_src; ?>" class="img-fluid logo-factchecking" alt="<?php echo $image_alt; ?>"/>
                ...
                <div class="fact-checking-newbox p-4 mb-5">
                    ...
                    <?php echo function_exists('get_field') ? get_field('post_fact_checking_verificamos') : ''; ?>
                </div>
            <?php endif;
        } else {
            the_content(
            ...
            if (!(in_category('diver-check') || in_category('1635'))) {
                ?>
                ...
                <h5 id="h-la-informaci-n-que-publicamos-en-divergentes-proviene-de-fuentes-contrastadas-debido-a-la-situaci-n-en-la-regi-n-muchas-veces-nos-vemos-obligados-a-protegerlas-bajo-seud-nimo-o-anonimato-desafortunadamente-algunos-gobiernos-de-la-regi-n-con-el-r-gimen-de-nicaragua-a-la-cabeza-no-ofrecen-informaci-n-o-censuran-a-los-medios-independientes-por-ello-a-pesar-de-solicitarlo-no-podemos-contar-con-versiones-oficiales-autorizadas-recurrimos-al-an-lisis-de-datos-a-las-fuentes-internas-an-nimas-o-las-limitadas-informaciones-de-los-medios-oficialistas-estas-son-las-condiciones-en-las-que-ejercemos-un-oficio-que-en-muchos-casos-nos-cuesta-la-seguridad-y-la-vida-seguiremos-informando">
                    <em>La información que publicamos en </em><strong><em>DIVERGENTES</em></strong><em> proviene de
                        ...
                        oficio que, en muchos casos, nos cuesta la seguridad y la vida. Seguiremos informando.</em></h5>
                <?php
            }
        if (in_category('1635')) {
            ?>
            <hr class="wp-block-separator has-alpha-channel-opacity">
            <h5 id="h-la-informaci-n-que-publicamos-en-divergentes-proviene-de-fuentes-contrastadas-debido-a-la-situaci-n-en-la-regi-n-muchas-veces-nos-vemos-obligados-a-protegerlas-bajo-seud-nimo-o-anonimato-desafortunadamente-algunos-gobiernos-de-la-regi-n-con-el-r-gimen-de-nicaragua-a-la-cabeza-no-ofrecen-informaci-n-o-censuran-a-los-medios-independientes-por-ello-a-pesar-de-solicitarlo-no-podemos-contar-con-versiones-oficiales-autorizadas-recurrimos-al-an-lisis-de-datos-a-las-fuentes-internas-an-nimas-o-las-limitadas-informaciones-de-los-medios-oficialistas-estas-son-las-condiciones-en-las-que-ejercemos-un-oficio-que-en-muchos-casos-nos-cuesta-la-seguridad-y-la-vida-seguiremos-informando">
                <em>The information we publish in </em><strong><em>DIVERGENTES</em></strong><em> comes from contrasted sources. Due to the situation in the region, many times, we are forced to protect them under pseudonymity or anonymity. Unfortunately, some governments in the region, including the Nicaraguan regime, do not provide information or censor independent media. For this reason, despite requesting it, we cannot rely on official, authorized versions. We resort to data analysis, anonymous internal sources, or limited information from the official media. These are the conditions under which we exercise a profession that, in many cases, costs us our safety and our lives. We will continue to report.</em></h5>
            <?php
        }
